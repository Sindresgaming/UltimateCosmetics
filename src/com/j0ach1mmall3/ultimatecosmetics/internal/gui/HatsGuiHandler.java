 ...
 */
public final class HatsGuiHandler extends GuiHandler {
    public HatsGuiHandler(Main plugin) {
        super(plugin);
    }

    public static void open(Player p, int page) {
        Player p1 = p;
        Hats config = plugin.getHats();
        CosmeticsAPI api = plugin.getApi();
        GUI gui = buildGui(config.getGuiName(), config.getGuiSize());
        for (HatStorage hat : config.getHats()) {
            int position = getRealPosition(hat.getPosition(), page, config.getGuiSize());
            if (position != -1) {
                ItemStack item = hat.getItem().clone();
                if (api.hasHat(p1)) {
                    if (api.getHat(p1).getHatStorage().getIdentifier().equals(hat.getIdentifier()))
                        item.addUnsafeEnchantment(glow, 1);
                }
                gui.setItem(position, item);
                if (config.isNoPermissionItemEnabled() && !Methods.hasPermission(p1, hat.getPermission())) {
                    gui.setItem(position, config.getNoPermissionItem(hat));
                }
            }
        }
    ...
    protected void handleClick(GUI gui, Player p, ItemStack item) {
        ...
        if (gui.getName().equals(Placeholders.parse(config.getGuiName(), p))) {
            ...
            HatStorage hat = api.getHatByItemStack(item);
            if (!Methods.hasPermission(p, hat.getPermission())) {
                plugin.informPlayerNoPermission(p, config.getNoPermissionMessage());
                return;
            }
            new HatImpl(p, hat).give();
